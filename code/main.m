...
ds = 1; % 0: KITTI, 1: Malaga, 2: parking, 3: indoor_ros, 4: indoor_img_notag
...
% outdoor_path = 'datasets/mydataset';
...
%     error = sum(reprojection_error)/size(reprojection_error,2) + error;
    
    if sum(is_valid_landmark) < 20
        disp(['reprojection_error is: ',num2str(reprojection_error)]);
...
%     if size(new_found_landmarks,2) < 8
%         disp(['Special case: size(new_found_landmarks,2) < 10; Number of is_reasonable_landmark: ',num2str(sum(new_found_landmarks))]);
%         new_found_landmarks = triangulated_candidates(: ,is_valid_landmark);
%         new_found_keypoints = state_curr.C(: ,is_valid_landmark);
%     end

    disp(['Number of added landmarks: ',num2str(sum(is_valid_landmark & is_reasonable_landmark))]);
%     if sum(is_valid_landmark & is_reasonable_landmark) < 2
%         i = i+1;
%         continue;
%     end
