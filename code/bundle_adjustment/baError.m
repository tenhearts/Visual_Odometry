...
plot_debug = false;
hidden_state = [HomogMatrix2twist(first_pose);HomogMatrix2twist(second_pose);hidden_state]; %前两个相机pose在参数传递进来的时候被移除了，在这里恢复原样
...
for i = 1:num_frames
    ...
    p_W_L = p_W_landmarks(:, landmark_indices); %在世界坐标系下的3D点坐标
    ...
    p_C_L = T_C_W(1:3,1:3) * p_W_L + ...
        repmat(T_C_W(1:3, end), [1 num_landmarks]); %在相机坐标系下的3D点坐标
