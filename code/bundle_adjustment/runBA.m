...
if with_pattern % provide the nonlinear optimization with information on
    ...
    num_frames = observations(1); %一共有多少个state，一共有多少个keyframe，12个
    num_observations = (numel(observations)-2-num_frames)/3; % 这12个frame中所有的keypoints个数
    ...
    pattern = spalloc(num_error_terms, numel(hidden_state)-2*6, num_error_terms * 9);
    % spalloc(m,n,nz): 创建一个大小为 m×n 的全零稀疏矩阵，该矩阵具有可存储 nz 个非零值的空间
    ...
    for frame_i = 1:num_frames
        ...
        if frame_i > 2 %add not first two poses as optimization variable
            pattern(error_i:error_i+2*num_keypoints_in_frame-1, (frame_i-3)*6+1:(frame_i-2)*6) = 1;
        end
...
options = optimoptions(@lsqnonlin, 'Display', 'iter', 'MaxIter', max_iter);
if with_pattern
